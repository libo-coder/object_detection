...
y_pre:
    ...
    2. 对于输出的 y1、y2、y3 而言，
       ...
       […, 4: 5] 指的是该框的置信度，
       […, 5: ]  指的是每个种类的预测概率。
    3. 现在的y_pre还是没有解码的，解码了之后才是真实图像上的情况。
...
def smooth_labels(y_true, label_smoothing, num_classes):
    ...
    其实Label Smoothing平滑就是将标签进行一个平滑，原始的标签是0、1，在平滑后变成0.005(如果是二分类)、0.995，
    也就是说对分类准确做了一点惩罚，让模型不可以分类的太准确，太准确容易过拟合。
    """
...
def box_ciou(b1, b2):
    ...
    ### 修改: libo
    v1 = torch.atan(b1_wh[..., 0] / torch.clamp(b1_wh[..., 1], min=1e-6))
    v2 = torch.atan(b2_wh[..., 0] / torch.clamp(b2_wh[..., 1], min=1e-6))
    v = (4 / (math.pi ** 2)) * torch.pow((v1 - v2), 2)
...
class YOLOLoss(nn.Module):
    """ loss 组成
    在计算loss的时候，实际上是 y_pre 和 y_true 之间的对比：
    1. y_pre 就是一幅图像经过网络之后的输出，内部含有三个特征层的内容；其需要解码才能够在图上作画
    2. y_true 就是一个真实图像中，它的每个真实框对应的(19,19)、(38,38)、(76,76)网格上的偏移位置、长宽与种类。其仍需要编码才能与 y_pred 的结构一致
    3. 实际上 y_pre 和 y_true 内容的 shape 都是
    ...
    def forward(self, input, targets=None):
        # input 为 bs, 3*(5+num_classes), 13, 13
        bs = input.size(0)      # 一共多少张图片
        in_h = input.size(2)    # 特征层的高
        in_w = input.size(3)    # 特征层的宽
    ...
    def get_target(self, target, anchors, in_w, in_h, ignore_threshold):
        bs = len(target)  # 计算一共有多少张图片
        anchor_index = [[0, 1, 2], [3, 4, 5], [6, 7, 8]][self.feature_length.index(in_w)]  # 获得先验框
        subtract_index = [0, 3, 6][self.feature_length.index(in_w)]
        mask = torch.zeros(bs, int(self.num_anchors / 3), in_h, in_w, requires_grad=False)  # 创建全是0或者全是1的阵列
...
class Generator(object):
    ...
    def get_random_data_with_Mosaic(self, annotation_line, input_shape, hue=.1, sat=1.5, val=1.5):
        ...
        for line in annotation_line:
            line_content = line.split()     # 每一行进行分割
            image = Image.open(line_content[0])
            image = image.convert("RGB")
            iw, ih = image.size         # 图片的大小
            box = np.array([np.array(list(map(int, box.split(',')))) for box in line_content[1:]])      # 保存框的位置
