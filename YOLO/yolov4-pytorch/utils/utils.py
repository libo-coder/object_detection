...
class DecodeBox(nn.Module):
    ...
    yolo3 的 3 个特征层分别将整幅图分为 19x19、38x38、76x76 的网格，每个网络点负责一个区域的检测。
    ...
    yolo3 的解码过程就是将每个网格点加上它对应的 x_offset 和 y_offset，加完后的结果就是预测框的中心，
    然后再利用 先验框和h、w结合 计算出预测框的长和宽。这样就能得到整个预测框的位置了。

    当然得到最终的预测结构后还要进行得分排序与非极大抑制筛选，这一部分基本上是所有目标检测通用的部分。
    不过该项目的处理方式与其它项目不同。其对于每一个类进行判别。
    ...
    def forward(self, input):
        # input 为 bs, 3*(1+4+num_classes), 13, 13
        batch_size = input.size(0)      # 一共多少张图片
        ...
        # 把先验框的尺寸调整成特征层大小的形式，计算出先验框在特征层上对应的宽高
...
def merge_bboxes(bboxes, cutx, cuty):
    ...
    实现思路：
        1. 每次读取四张图片
        2. 分别对四张图片进行翻转、缩放、色域变化等，并且按照四个方向摆好
        3. 进行图片的组合和框的组合
    """
    merge_bbox = []
