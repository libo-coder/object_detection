    ...
    def __getitem__(self, index):
        ...
        for i in range(use_mixup + 1):
            if i != 0:
                ...
                bboxes = np.array(self.truth.get(img_path), dtype=np.float)
                img_path = os.path.join(self.cfg.dataset_dir, img_path)
            print(img_path)     # /home/room/data1/libo/yolov4-pytorch2/VOCdevkit_lb/VOC0807/JPEGImages/000007      type = str
...
if __name__ == "__main__":
    from cfg_lb import Cfg
    import matplotlib.pyplot as plt

    random.seed(2020)
    np.random.seed(2020)
    # Cfg.dataset_dir = '/mnt/e/Dataset'
    Cfg.dataset_dir = '/home/room/data1/libo/yolov4-pytorch2/VOCdevkit_lb/VOC0807/JPEGImages'
    dataset = Yolo_dataset(Cfg.train_label, Cfg)
    for i in range(100):
        out_img, out_bboxes = dataset.__getitem__(i)
        a = draw_box(out_img.copy(), out_bboxes.astype(np.int32))
        cv2.imwrite('./debug/' + str(i) + '.jpg', a.astype(np.int32))
