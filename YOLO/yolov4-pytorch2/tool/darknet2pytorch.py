...
class Darknet(nn.Module):
    def __init__(self, cfgfile, inference=False):
        """
        在创建 Darknet() 对象过程中，会根据 传入的 cfg 文件初始化工作
        主要是 cfg 文件的解析，提取 cfg 中的每个 block；网络结构的构建；
        """
        super(Darknet, self).__init__()
        self.inference = inference
        self.training = not self.inference

        self.blocks = parse_cfg(cfgfile)    # tool/config.py 函数中
    ...
    def create_network(self, blocks):
        """
        根据解析 cfg 得到的 blocks 构建网络，
        先创建一个 ModuleList 模型列表，为每个 block 创建个 Sequential(),
        将每个 block 中的卷积操作，BN 操作，激活操作都放到这个 Sequential() 中；
        可以理解为每个 block 对应一个 Sequential() 。
        """
    ...
    def load_weights(self, weightfile):
        ...
        buf = np.fromfile(fp, dtype=np.float32)     # buf 是加载后的 yolo4.weights 数据内容
        fp.close()
        ...
        for block in self.blocks:
            ...
            if block['type'] == 'net':
                ...
                batch_normalize = int(block['batch_normalize'])
                if batch_normalize:
                    start = load_conv_bn(buf, start, model[0], model[1])        # load_conv_bn 在 tool/config.py 中
                else:
            ...
            # 下面的几个 block 在训练过程中是不会生成权重值的，所以不用从 yolo4.weights 中取值
            elif block['type'] == 'maxpool':
                pass
            elif block['type'] == 'reorg':
                pass
