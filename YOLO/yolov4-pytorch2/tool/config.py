...
def load_conv_bn(buf, start, conv_model, bn_model):     # start: 64
    """ 针对从 buf 中读取第一个 block 数值，该 block 包含 BN2 和 conv，以及 mish 激活操作 """
    num_w = conv_model.weight.numel()       # num_w: 864
    num_b = bn_model.bias.numel()       # num_b: 32
    bn_model.bias.data.copy_(torch.from_numpy(buf[start:start + num_b]))    # 0:32 BN2 的 b 值
    start = start + num_b
    bn_model.weight.data.copy_(torch.from_numpy(buf[start:start + num_b]))  # 32:64 BN2 的 a 值
    start = start + num_b
    bn_model.running_mean.copy_(torch.from_numpy(buf[start:start + num_b]))     # 64:92 BN2 的 mean 值
    start = start + num_b
    bn_model.running_var.copy_(torch.from_numpy(buf[start:start + num_b]))      # 92:128 BN2 的 var 值
    start = start + num_b
    # 128:992 conv 的 weight 值
    conv_model.weight.data.copy_(torch.from_numpy(buf[start:start + num_w]).reshape(conv_model.weight.data.shape))
    start = start + num_w       # 返回 start=992
    return start
